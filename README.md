# README翻译（严格逐章对齐 UE5.3）

> 说明：本文件按 `README.md`（UE5.3）锚点章节顺序编排。
> 已复用 `README_cn.md` 的已译章节，并补齐其缺失章节的中文翻译。

﻿# GAS 文档
我通过一个简单的多人游戏示例项目来了解虚幻引擎 5 的 GameplayAbilitySystem 插件 (GAS)。这不是官方文档，这个项目和我本人都不隶属于 Epic Games。我不保证这些信息的准确性。

本文档的目的是解释 GAS 中的主要概念和类，并根据我的经验提供一些附加评论。社区用户中有很多关于 GAS 的“部落知识”，我的目标是在这里分享我的所有知识。

示例项目和文档适用于**虚幻引擎 5.3** (UE5)。本文档有针对旧版本虚幻引擎的分支，但它们不再受支持，并且可能存在错误或过时的信息。请使用与您的引擎版本匹配的分支。

[GASShooter](https://github.com/tranek/GASShooter) 是一个姊妹示例项目，展示了 GAS 在多人 FPS/TPS 中的进阶用法。
...
> 1. [Intro to the GameplayAbilitySystem Plugin (步入游戏能力系统插件)](#intro)
> 2. [Sample Project (样例项目)](#sp)
> 3. [Setting Up a Project Using GAS (使用GAS创建一个项目)](#setup)
> 4. [Concepts (GAS概念)](#concepts)  
>    4.1 [Ability System Component (能力系统组件)](#concepts-asc)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.1.1 [Replication Mode (同步模式)](#concepts-asc-rm)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.1.2 [Setup and Initialization (设置和初始化)](#concepts-asc-setup)  
>    4.2 [Gameplay Tags (游戏标签)](#concepts-gt)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.2.1 [Responding to Changes in Gameplay Tags (响应游戏标签变化)](#concepts-gt-change)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.2.2 [Loading Gameplay Tags from Plugin .ini Files (从插件配置文件加载游戏标签)](#concepts-gt-loadfromplugin)  
>    4.3 [Attributes (属性)](#concepts-a)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.3.1 [Attribute Definition (属性定义)](#concepts-a-definition)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.3.2 [BaseValue vs CurrentValue (基本值与当前值)](#concepts-a-value)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.3.3 [Meta Attributes (元属性)](#concepts-a-meta)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.3.4 [Responding to Attribute Changes (响应属性变化)](#concepts-a-changes)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.3.5 [Derived Attributes (自动推导属性)](#concepts-a-derived)  
>    4.4 [Attribute Set (属性集)](#concepts-as)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.1 [Attribute Set Definition (定义属性集)](#concepts-as-definition)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2 [Attribute Set Design (设计属性集)](#concepts-as-design)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2.1 [Subcomponents with Individual Attributes (使用独立属性的子组件)](#concepts-as-design-subcomponents)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2.2 [Adding and Removing AttributeSets at Runtime (运行时添加和移除属性集)](#concepts-as-design-addremoveruntime)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2.3 [Item Attributes (Weapon Ammo) (物品属性（武器弹药）)](#concepts-as-design-itemattributes)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2.3.1 [Plain Floats on the Item (在物品中使用普通浮点数)](#concepts-as-design-itemattributes-plainfloats)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2.3.2 [`AttributeSet` on the Item (在物品中使用属性集)](#concepts-as-design-itemattributes-attributeset)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.2.3.3 [`ASC` on the Item (在物品中使用单独的ASC)](#concepts-as-design-itemattributes-asc)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.3 [Defining Attributes (定义属性)](#concepts-as-attributes)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.4 [Initializing Attributes (初始化属性)](#concepts-as-init)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.5 [PreAttributeChange() (预属性更改())](#concepts-as-preattributechange)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.6 [PostGameplayEffectExecute() (后置游戏效果执行回调)](#concepts-as-postgameplayeffectexecute)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.4.7 [OnAttributeAggregatorCreated() (属性聚合器创建回调)](#concepts-as-onattributeaggregatorcreated)  
>    4.5 [Gameplay Effects (游戏效果)](#concepts-ge)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.1 [Gameplay Effect Definition (定义游戏效果)](#concepts-ge-definition)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.2 [Applying Gameplay Effects (应用游戏效果)](#concepts-ge-applying)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.3 [Removing Gameplay Effects (移除游戏效果)](#concepts-ga-removing)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.4 [Gameplay Effect Modifiers (游戏效果修改器)](#concepts-ge-mods)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.4.1 [Multiply and Divide Modifiers (乘法与除法修改器)](#concepts-ge-mods-multiplydivide)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.4.2 [Gameplay Tags on Modifiers (修改器的游戏标签)](#concepts-ge-mods-gameplaytags)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.5 [Stacking Gameplay Effects (游戏效果堆叠)](#concepts-ge-stacking)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.6 [Granted Abilities (授予能力)](#concepts-ge-ga)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.7 [Gameplay Effect Tags (游戏效果标签)](#concepts-ge-tags)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.8 [Immunity (免疫)](#concepts-ge-immunity)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.9 [Gameplay Effect Spec (游戏效果规格)](#concepts-ge-spec)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.9.1 [SetByCallers (由调用者设置)](#concepts-ge-spec-setbycaller)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.10 [Gameplay Effect Context (游戏效果背景)](#concepts-ge-context)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.11 [Modifier Magnitude Calculation (修饰符大小计算)](#concepts-ge-mmc)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.12 [Gameplay Effect Execution Calculation (游戏效果执行计算)](#concepts-ge-ec)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.12.1 [Sending Data to Execution Calculations (向执行计算传递数据)](#concepts-ge-ec-senddata)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.12.1.1 [SetByCaller (由调用者设置)](#concepts-ge-ec-senddata-setbycaller)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.12.1.2 [Backing Data Attribute Calculation Modifier (后备数据属性计算修改器)](#concepts-ge-ec-senddata-backingdataattribute)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.12.1.3 [Backing Data Temporary Variable Calculation Modifier (后备数据临时变量计算修改器)](#concepts-ge-ec-senddata-backingdatatempvariable)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.12.1.4 [Gameplay Effect Context (游戏效果背景)](#concepts-ge-ec-senddata-effectcontext)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.13 [Custom Application Requirement (自定义应用需求)](#concepts-ge-car)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.14 [Cost Gameplay Effect (消耗型游戏效果)](#concepts-ge-cost)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.15 [Cooldown Gameplay Effect (冷却型游戏效果)](#concepts-ge-cooldown)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.15.1 [Get the Cooldown Gameplay Effect's Remaining Time (获取冷却型游戏效果的剩余时间)](#concepts-ge-cooldown-tr)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.15.2 [Listening for Cooldown Begin and End (监听冷却开始和结束)](#concepts-ge-cooldown-listen)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.15.3 [Predicting Cooldowns (预测冷却时间)](#concepts-ge-cooldown-prediction)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.16 [Changing Active Gameplay Effect Duration (修改已激活游戏效果的持续时间)](#concepts-ge-duration)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.17 [Creating Dynamic Gameplay Effects at Runtime (运行时创建动态游戏效果)](#concepts-ge-dynamic)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.5.18 [Gameplay Effect Containers (游戏效果容器)](#concepts-ge-containers)  
>    4.6 [Gameplay Abilities (游戏能力)](#concepts-ga)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.1 [Gameplay Ability Definition (游戏能力定义)](#concepts-ga-definition)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.1.1 [Replication Policy (复制策略)](#concepts-ga-definition-reppolicy)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.1.2 [Server Respects Remote Ability Cancellation (服务器尊重远程能力取消)](#concepts-ga-definition-remotecancel)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.1.3 [Replicate Input Directly (直接复制输入)](#concepts-ga-definition-repinputdirectly)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.2 [Binding Input to the ASC (绑定输入到ASC)](#concepts-ga-input)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.2.1 [Binding to Input without Activating Abilities (绑定输入时不激活能力)](#concepts-ga-input-noactivate)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.3 [Granting Abilities (授予能力)](#concepts-ga-granting)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.4 [Activating Abilities (激活能力)](#concepts-ga-activating)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.4.1 [Passive Abilities (被动能力)](#concepts-ga-activating-passive)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.4.2 [Activation Failed Tags (激活失败标签)](#concepts-ga-activating-failedtags)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.5 [Canceling Abilities (取消能力)](#concepts-ga-cancelabilities)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.6 [Getting Active Abilities (获取激活的能力)](#concepts-ga-definition-activeability)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.7 [Instancing Policy (实例化策略)](#concepts-ga-instancing)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.8 [Net Execution Policy (网络执行策略)](#concepts-ga-net)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.9 [Ability Tags (能力标签)](#concepts-ga-tags)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.10 [Gameplay Ability Spec (游戏能力规格)](#concepts-ga-spec)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.11 [Passing Data to Abilities (向能力传递数据)](#concepts-ga-data)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.12 [Ability Cost and Cooldown (能力消耗与冷却)](#concepts-ga-commit)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.13 [Leveling Up Abilities (升级能力)](#concepts-ga-leveling)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.14 [Ability Sets (能力集合)](#concepts-ga-sets)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.15 [Ability Batching (能力批处理)](#concepts-ga-batching)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.6.16 [Net Security Policy (网络安全策略)](#concepts-ga-netsecuritypolicy)  
>    4.7 [Ability Tasks (能力任务)](#concepts-at)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.7.1 [Ability Task Definition (自定义能力任务)](#concepts-at-definition)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.7.2 [Custom Ability Tasks (自定义能力任务)](#concepts-at-custom-abilitytasks)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.7.3 [Using Ability Tasks (使用能力任务)](#concepts-at-using)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.7.4 [Root Motion Source Ability Tasks (根动源能力任务)](#concepts-at-rms)  
>    4.8 [Gameplay Cues (游戏提示)](#concepts-gc)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.1 [Gameplay Cue Definition (游戏提示定义)](#concepts-gc-definition)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.2 [Triggering Gameplay Cues (触发游戏提示)](#concepts-gc-trigger)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.3 [Local Gameplay Cues (本地游戏提示)](#concepts-gc-local)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.4 [Gameplay Cue Parameters (游戏提示参数)](#concepts-gc-parameters)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.5 [Gameplay Cue Manager (游戏提示管理器)](#concepts-gc-manager)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.6 [Prevent Gameplay Cues from Firing (阻止游戏提示触发)](#concepts-gc-prevention)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.7 [Gameplay Cue Batching (游戏提示批处理)](#concepts-gc-batching)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.7.1 [Manual RPC (手动RPC)](#concepts-gc-batching-manualrpc)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.7.2 [Multiple GCs on one GE (单个游戏效果中的多个游戏提示)](#concepts-gc-batching-gcsonge)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.8 [Gameplay Cue Events (游戏提示事件)](#concepts-gc-events)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.8.9 [Gameplay Cue Reliability (游戏提示可靠性)](#concepts-gc-reliability)  
>    4.9 [Ability System Globals (能力系统全局)](#concepts-asg)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.9.1 [InitGlobalData() (初始化全局数据())](#concepts-asg-initglobaldata)  
>    4.10 [Prediction (预测)](#concepts-p)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.10.1 [Prediction Key (预测键)](#concepts-p-key)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.10.2 [Creating New Prediction Windows in Abilities (在能力中创建新的预测窗口)](#concepts-p-windows)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.10.3 [Predictively Spawning Actors (预测性生成角色)](#concepts-p-spawn)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.10.4 [Future of Prediction in GAS (GAS中预测的未来)](#concepts-p-future)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.10.5 [Network Prediction Plugin (网络预测插件)](#concepts-p-npp)  
>    4.11 [Targeting (瞄准)](#concepts-targeting)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.11.1 [Target Data (目标数据)](#concepts-targeting-data)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.11.2 [Target Actors (目标演员)](#concepts-targeting-actors)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.11.3 [Target Data Filters (目标数据过滤器)](#concepts-target-data-filters)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.11.4 [Gameplay Ability World Reticles (游戏能力世界十字线)](#concepts-targeting-reticles)  
>    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.11.5 [Gameplay Effect Containers Targeting (游戏效果容器定位)](#concepts-targeting-containers)  
> 5. [Commonly Implemented Abilities and Effects (常见能力与效果)](#cae)  
>    5.1 [Stun (眩晕)](#cae-stun)  
>    5.2 [Sprint (奔跑)](#cae-sprint)  
>    5.3 [Aim Down Sights (举镜瞄准)](#cae-ads)  
>    5.4 [Lifesteal (生命偷取)](#cae-ls)  
>    5.5 [Generating a Random Number on Client and Server (在客户端和服务端中生成随机数)](#cae-random)  
>    5.6 [Critical Hits (暴击)](#cae-crit)  
>    5.7 [Non-Stacking Gameplay Effects but Only the Greatest Magnitude Actually Affects the Target (非堆叠游戏效果中仅最大强度会影响目标)](#cae-nonstackingge)  
>    5.8 [Generate Target Data While Game is Paused (游戏暂停时生成目标数据)](#cae-paused)  
>    5.9 [One Button Interaction System (单键交互系统)](#cae-onebuttoninteractionsystem)  
> 6. [Debugging GAS (调试GAS)](#debugging)  
>    6.1 [showdebug abilitysystem (能力系统调试页)](#debugging-sd)  
>    6.2 [Gameplay Debugger (游戏调试器)](#debugging-gd)  
>    6.3 [GAS Logging (GAS日志)](#debugging-log)  
> 7. [Optimizations (优化)](#optimizations)  
>    7.1 [Ability Batching (能力批处理)](#optimizations-abilitybatching)  
>    7.2 [Gameplay Cue Batching (游戏提示批处理)](#optimizations-gameplaycuebatching)  
>    7.3 [AbilitySystemComponent Replication Mode (能力系统组件同步模式)](#optimizations-ascreplicationmode)  
>    7.4 [Attribute Proxy Replication (属性代理同步)](#optimizations-attributeproxyreplication)  
>    7.5 [ASC Lazy Loading (ASC懒加载)](#optimizations-asclazyloading)  
> 8. [Quality of Life Suggestions (生活质量建议)](#qol)  
>    8.1 [Gameplay Effect Containers (游戏效果容器)](#qol-gameplayeffectcontainers)  
>    8.2 [Blueprint AsyncTasks to Bind to ASC Delegates (将蓝图异步任务绑定到ASC委托)](#qol-asynctasksascdelegates)  
> 9. [Troubleshooting (疑难解答)](#troubleshooting)  
>    9.1 [`LogAbilitySystem: Warning: Can't activate LocalOnly or LocalPredicted ability %s when not local!` (`LogAbilitySystem`警告：非本地时无法激活本地能力)](#troubleshooting-notlocal)  
>    9.2 [`ScriptStructCache` errors (`ScriptStructCache` 错误)](#troubleshooting-scriptstructcache)  
>    9.3 [Animation Montages are not replicating to clients (动画蒙太奇不能同步到客户端)](#troubleshooting-replicatinganimmontages)  
>    9.4 [Duplicating Blueprint Actors is setting AttributeSets to nullptr (复制的蓝图角色会将属性集设为空指针)](#troubleshooting-duplicatingblueprintactors)  
>    9.5 [unresolved external symbol UEPushModelPrivate::MarkPropertyDirty(int,int) (未解析的外部符号 `UEPushModelPrivate::MarkPropertyDirty(int,int)`)](#troubleshooting-unresolvedexternalsymbolmarkpropertydirty)  
>    9.6 [Enum names are now represented by path name (枚举名称现在使用路径名表示)](#troubleshooting-enumnamesarenowpathnames)  
> 10. [Common GAS Acronyms (ASC常用术语缩略)](#acronyms)
> 11. [Other Resources (其他资源)](#resources)  
>     11.1 [Q&A With Epic Game's Dave Ratti (与史诗游戏开发者戴夫·拉蒂的问答)](#resources-daveratti)  
>     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;11.1.1 [Community Questions 1 (社区问题 1)](#resources-daveratti-community1)  
>     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;11.1.2 [Community Questions 2 (社区问题 2)](#resources-daveratti-community2)  
> 12. [GAS Changelog (GAS更新日志)](#changelog)  
>     * [5.3 (版本 5.3)](#changelog-5.3)  
>     * [5.2 (版本 5.2)](#changelog-5.2)  
>     * [5.1 (版本 5.1)](#changelog-5.1)  
>     * [5.0 (版本 5.0)](#changelog-5.0)  
>     * [4.27 (版本 4.27)](#changelog-4.27)  
>     * [4.26 (版本 4.26)](#changelog-4.26)  
>     * [4.25.1 (版本 4.25.1)](#changelog-4.25.1)  
>     * [4.25 (版本 4.25)](#changelog-4.25)  
>     * [4.24 (版本 4.24)](#changelog-4.24)
...
> Gameplay Ability System 是一个高度灵活的框架，可用于构建你可能会在RPG或MOBA游戏中看到的技能和属性类型。你可以构建可供游戏中的角色使用的动作或被动技能，使这些动作导致各种属性累积或损耗的状态效果，实现约束这些动作使用的"冷却"计时器或资源消耗，更改技能等级及每个技能等级的技能效果，激活粒子或音效，等等。简单来说，此系统可帮助你在任何现代RPG或MOBA游戏中设计、实现及高效关联各种游戏中的技能，既包括跳跃等简单技能，也包括你喜欢的角色的复杂技能集。
...
<a id="concepts-gt-loadfromplugin"></a>

### 4.2.2 从插件 .ini 文件加载 Gameplay Tags

如果你编写了带有独立 `.ini` 的插件，并在其中维护 `GameplayTags`，可以在插件的 `StartupModule()` 中把该插件的 `GameplayTag` 配置目录注册到标签管理器。
...
如果该插件被启用，引擎启动时会扫描 `Plugins\CommonConversation\Config\Tags` 目录，并将其中声明了 `GameplayTags` 的 `.ini` 文件加载进项目。
...
<a id="concepts-as-design-subcomponents"></a>
...
<a id="concepts-as-design-addremoveruntime"></a>
...
<a id="concepts-as-design-itemattributes"></a>
...
<a id="concepts-as-design-itemattributes-plainfloats"></a>
...
<a id="concepts-as-design-itemattributes-attributeset"></a>
...
<a id="concepts-as-design-itemattributes-asc"></a>
...
<a id="concepts-as-preattributechange"></a>
...
<a id="concepts-as-postgameplayeffectexecute"></a>
...
<a id="concepts-as-onattributeaggregatorcreated"></a>
...
<a id="concepts-ge-mods-multiplydivide"></a>
...
<a id="concepts-ge-mods-gameplaytags"></a>
...
<a id="concepts-ge-spec-setbycaller"></a>
...
<a id="concepts-ge-ec-senddata"></a>
...
<a id="concepts-ge-ec-senddata-setbycaller"></a>
...
<a id="concepts-ge-ec-senddata-backingdataattribute"></a>
...
<a id="concepts-ge-ec-senddata-backingdatatempvariable"></a>
...
<a id="concepts-ge-ec-senddata-effectcontext"></a>
...
<a id="concepts-ge-cooldown-tr"></a>
...
<a id="concepts-ge-cooldown-listen"></a>
...
<a id="concepts-ge-cooldown-prediction"></a>
...
<a id="concepts-ga-definition-reppolicy"></a>
...
<a id="concepts-ga-definition-remotecancel"></a>
...
<a id="concepts-ga-definition-repinputdirectly"></a>
...
<a id="concepts-ga-input-noactivate"></a>
...
<a id="concepts-ga-activating-passive"></a>
...
<a id="concepts-ga-activating-failedtags"></a>

##### 4.6.4.2 激活失败标签

Ability 内置了一套“激活失败原因”上报逻辑。要启用它，你需要先在项目里定义对应的 `GameplayTag`。

先把以下标签（或同语义的自定义命名）加入项目：
...
然后在 [`GASDocumentation\Config\DefaultGame.ini`](https://github.com/tranek/GASDocumentation/blob/master/Config/DefaultGame.ini#L8-L13) 中配置这些映射：
...
配置完成后，每当能力激活失败，对应的失败 Tag 会出现在输出日志里，也可以在 `showdebug AbilitySystem` HUD 中看到：
...
<a id="concepts-ga-cancelabilities"></a>
...
<a id="concepts-ga-definition-activeability"></a>
...
<a id="concepts-ga-netsecuritypolicy"></a>
...
<a id="concepts-at-custom-abilitytasks"></a>
...
<a id="concepts-gc-batching-manualrpc"></a>
...
<a id="concepts-gc-batching-gcsonge"></a>
...
<a id="concepts-gc-events"></a>

#### 4.8.8 Gameplay Cue 事件

`GameplayCues` 会响应 `EGameplayCueEvents` 中的特定事件：

| `EGameplayCueEvent` | 说明                                                                                                                                                            |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OnActive`          | 当 `GameplayCue` 被激活（添加）时触发。                                                                                                                                   |
| `WhileActive`       | 当 `GameplayCue` 处于激活态时触发。即使它不是“刚应用”，例如中途加入玩家，也会触发。它不是 `Tick`，通常只在 `GameplayCueNotify_Actor` 被添加或变为相关时调用一次。若你确实需要逐帧逻辑，请直接使用 `GameplayCueNotify_Actor::Tick()`。 |
| `Removed`           | 当 `GameplayCue` 被移除时触发。蓝图侧对应 `OnRemove`。                                                                                                                      |
| `Executed`          | 当 `GameplayCue` 被执行时触发（例如瞬时效果或周期执行）。蓝图侧对应 `OnExecute`。                                                                                                        |

实践建议：
使用 `OnActive` 放置“开场瞬时表现”（错过也无妨）；使用 `WhileActive` 放置“持续表现”（后加入玩家也应看到）；`OnRemove` 负责清理前两者创建的内容。

例如 MOBA 防御塔爆炸：首次爆炸粒子和音效放到 `OnActive`，爆炸后持续燃烧特效放到 `WhileActive`。这样后加入玩家不会重放“爆炸瞬间”，但仍能看到持续火焰。每次 Actor 进入 `GameplayCueNotify_Actor` 的相关性范围会触发 `WhileActive`，离开范围会触发 `OnRemove`。

**[⬆ 返回目录](#table-of-contents)**

<a id="concepts-gc-reliability"></a>

#### 4.8.9 Gameplay Cue 可靠性

总体上，`GameplayCues` 应视为**不可靠机制**，不适合承载直接影响玩法结果的关键逻辑。

**执行型 `GameplayCues`：**
这类 `GameplayCues` 通过不可靠多播触发，天然不可靠。

**由 `GameplayEffects` 触发的 `GameplayCues`：**

* Autonomous Proxy 可可靠收到 `OnActive`、`WhileActive`、`OnRemove`
  `FActiveGameplayEffectsContainer::NetDeltaSerialize()` 会调用 `UAbilitySystemComponent::HandleDeferredGameplayCues()` 触发 `OnActive` 与 `WhileActive`；`FActiveGameplayEffectsContainer::RemoveActiveGameplayEffectGrantedTagsAndModifiers()` 会触发 `OnRemoved`。
* Simulated Proxy 可可靠收到 `WhileActive`、`OnRemove`
  `UAbilitySystemComponent::MinimalReplicationGameplayCues` 的复制逻辑会触发 `WhileActive` 与 `OnRemove`，而 `OnActive` 仍由不可靠多播触发。

**不通过 `GameplayEffect` 直接触发的 `GameplayCues`：**

* Autonomous Proxy 仅 `OnRemove` 可可靠收到
  `OnActive` 与 `WhileActive` 由不可靠多播触发。
* Simulated Proxy 可可靠收到 `WhileActive`、`OnRemove`
  同样由 `UAbilitySystemComponent::MinimalReplicationGameplayCues` 复制触发；`OnActive` 仍是不可靠多播。

如果你需要 `GameplayCue` 表现“可靠”，建议通过 `GameplayEffect` 触发，并把持续特效放在 `WhileActive`，把清理逻辑放在 `OnRemove`。
...
<a id="concepts-asg-initglobaldata"></a>
...
<a id="concepts-targeting"></a>

### 4.11 Targeting

<a id="concepts-targeting-data"></a>
...
<a id="concepts-targeting-actors"></a>
...
<a id="concepts-target-data-filters"></a>
...
<a id="concepts-targeting-reticles"></a>
...
<a id="concepts-targeting-containers"></a>
...
<a id="cae-onebuttoninteractionsystem"></a>
...
<a id="optimizations-abilitybatching"></a>
...
<a id="optimizations-gameplaycuebatching"></a>
...
<a id="optimizations-ascreplicationmode"></a>
...
<a id="optimizations-attributeproxyreplication"></a>
...
<a id="optimizations-asclazyloading"></a>
...
<a id="qol-gameplayeffectcontainers"></a>
...
<a id="qol-asynctasksascdelegates"></a>
...
<a id="troubleshooting-notlocal"></a>
...
<a id="troubleshooting-scriptstructcache"></a>
...
<a id="troubleshooting-replicatinganimmontages"></a>
...
<a id="troubleshooting-duplicatingblueprintactors"></a>
...
<a id="troubleshooting-unresolvedexternalsymbolmarkpropertydirty"></a>
...
常见触发原因是在 `FFastArraySerializer` 上调用了 `MarkItemDirty()`。例如更新冷却时间时修改 `ActiveGameplayEffect`：

```c++
ActiveGameplayEffects.MarkItemDirty(*AGE);
```

本质问题是 `WITH_PUSH_MODEL` 在不同编译单元中被不一致地定义：
`PushModelMacros.h` 看到的是 0，而其他若干位置是 1；导致 `PushModel.h` 与 `PushModel.cpp` 的视图不一致。

解决方法：在项目 `Build.cs` 的 `PublicDependencyModuleNames` 中加入 `NetCore`。

**[⬆ 返回目录](#table-of-contents)**

<a id="troubleshooting-enumnamesarenowpathnames"></a>
...
从 UE 5.1 开始，`BindAbilityActivationToInputComponent()` 的旧构造方式（传 `FString` 枚举名）被弃用。现在应改为传入 `FTopLevelAssetPath`。
...
可参考：`Engine\Source\Runtime\CoreUObject\Public\UObject\TopLevelAssetPath.h`。
...
<a id="resources-daveratti"></a>

### 11.1 Epic Games 的 Dave Ratti 问答

<a id="resources-daveratti-community1"></a>

#### 11.1.1 社区问题 1

[Dave Ratti 回答 Unreal Slackers Discord Server 社区有关 GAS 的问题](https://epicgames.ent.box.com/s/m1egifkxv3he3u3xezb9hzbgroxyhx89):

1. 我们如何在 `GameplayAbilities` 之外按需创建范围预测窗口？例如，一发即忘射弹如何在击中敌人时局部预测伤害`GameplayEffect`？
...
> 
> 通过这种模式，PredictionKey 从服务器“弹回”并通过 UAbilitySystemComponent::ReplicatedPredictionKeyMap（复制属性）返回客户端。一旦密钥从服务器复制回来，客户端就能够撤消所有本地预测副作用（GameplayCues、GameplayEffects）：复制的版本*将在那里*，如果不存在，则这是一个错误预测。准确地知道何时消除预测副作用至关重要：如果太早，您会看到差距，如果太晚，您将看到“双重”。（请注意，这是指状态预测，例如基于持续时间的游戏效果的循环 GameplayCue。“突发”GameplayCues 和即时游戏效果永远不会“撤消”或回滚。如果存在与它们关联的预测键，它们只会在客户端上被跳过）。
> 
> 为了进一步切中要害：至关重要的是，预测操作是服务器不会自行执行的操作，而是仅在客户端告诉它们时才执行。因此，通用的“按需创建密钥并告诉服务器以便我可以运行某些内容”是行不通的，除非该“某些内容”是服务器仅在客户端告知后才会执行的操作。
> 
> 回到最初的问题：例如“发射后不再追踪的抛射体（fire-and-forget projectile）”。Paragon 和 Fortnite 都有使用 GameplayCues 的投射物 Actor 类。然而，我们不使用预测密钥系统来执行这些操作。相反，我们采用“非复制 GameplayCue”的做法：只在本地触发，服务器完全跳过。本质上，所有这些都是对 UGameplayCueManager::HandleGameplayCue 的直接调用。它们不通过 UAbilitySystemComponent 进行路由，因此不会进行预测密钥检查/提前返回。
> 
> 非复制 GameplayCues 的缺点是，它们没有被复制。因此，由射弹类/蓝图来确保调用这些函数的代码路径在每个人上运行。我们有启动（在 BeginPlay 中调用）、爆炸、撞墙/角色等提示。
> 
> 这类事件本来就由客户端产生，因此调用非复制 GameplayCue 问题不大。复杂的蓝图可能很棘手，并且需要作者确保他们了解什么在哪里运行。

2. 当使用 `WaitNetSync` `AbilityTask` 和 `OnlyServerWait` 在本地预测的 `GameplayAbility` 中创建范围预测窗口时，玩家是否可能通过延迟到服务器的数据包来控制 `GameplayAbility` 时间来作弊，因为服务器正在等待带有预测密钥的 RPC？Paragon 或 Fortnite 中是否曾出现过这个问题？如果是，Epic 采取了什么措施来解决这个问题？
...
> 
> 但我不相信《堡垒之夜》会使用这样的东西。《堡垒之夜》中的武器能力经过特殊处理，批量发送到单个《堡垒之夜》特定的 RPC：一个 RPC 可以激活该能力、提供目标数据并结束该能力。因此，在大逃杀中，武器能力本质上不会受到此影响。
> 
> 我的看法是，这可能是可以在整个系统范围内解决的问题，但我认为我们不会很快做出改变。现场修复 WaitNetSync 以包括您提到的情况的最大延迟可能是一项合理的任务，但同样 - 我们不太可能在不久的将来这样做。

3. Paragon 和 Fortnite 使用了哪一个`EGameplayEffectReplicationMode`？Epic 对于何时使用它们的建议是什么？

> 这两款游戏本质上都对玩家控制的角色使用混合模式，对人工智能控制的角色使用最小模式（人工智能小兵、丛林小兵、人工智能尸壳等）。这是我建议大多数人在多人游戏中使用该系统的方式。在项目中越早设置这些越好。
> 
> 《堡垒之夜》在优化方面更进一步。实际上，它根本不会为模拟代理复制 UAbilitySystemComponent。组件和属性子对象在拥有的 Fortnite 玩家状态类上的 ::ReplicateSubobjects() 内跳过。我们确实将最基本的复制数据从能力系统组件推送到 pawn 本身的结构（基本上，我们在位掩码中复制的属性值的子集和标签的白名单子集）。我们称之为“代理”。在接收端，我们获取在 pawn 上复制的代理数据，并将其推回到玩家状态的能力系统组件中。因此，FNBR 中的每个玩家确实都有一个 ASC，它只是不直接复制：相反，它通过 pawn 上的最小代理结构复制数据，然后路由回接收端的 ASC。这是一个优点，因为它 A) 更小的数据集 B) 利用了典当相关性。
> 
> 我不确定自那时以来所做的其他服务器端优化（复制图等）是否仍然有必要，而且它不是最可维护的模式。

4. 由于我们无法按照 `GameplayPrediction.h` 预测 `GameplayEffects` 的删除，是否有任何策略可以减轻延迟对删除 `GameplayEffects` 的影响？例如，当移除移动速度较慢时，我们目前必须等待服务器复制 `GameplayEffect` 移除操作，从而捕捉到玩家的角色位置。

> 这是一个很难的问题，我没有一个好的答案。我们通常通过公差和平滑来回避这些问题。我完全同意能力系统和与角色移动系统的精确同步不是一个好地方，也是我们确实想要修复的地方。
> 
> 我有一个允许预测性删除 GE 的架子，但在必须继续之前永远无法解决所有边缘情况。但这并不能解决所有问题，因为角色移动仍然有一个内部保存的移动缓冲区，它对能力系统和可能的移动速度修改器等一无所知。即使在无法预测 GE 移除的情况下，仍然有可能进入校正反馈循环。
> 
> 如果你认为你遇到了真正绝望的情况，你可以预测性地添加一个会抑制你移动速度的 GE。我自己从来没有这样做过，但之前已经对此进行了理论分析。它可能能够帮助解决某一类问题。

5. 我们知道 `AbilitySystemComponent` 存在于 Paragon 和 Fortnite 中的 `PlayerState` 上，以及 Action RPG Sample 中的 `Character` 上。Epic 的内部规则、指南或建议是什么，关于 AbilitySystemComponent 应该存在的位置，以及它的 `Owner` 应该是什么？

> 一般来说，我会说任何不需要重生的东西都应该让所有者和阿凡达演员是同一件事。任何 AI 敌人、建筑物、世界道具等。
> 
> 任何重生的东西都应该有不同的所有者和头像，这样能力系统组件就不需要在重生后保存/重新创建/恢复。PlayerState 是逻辑选择，它被复制到所有客户端（而 PlayerController 则不然）。缺点是 PlayerStates 总是相关的，因此您可能会在 100 名玩家的游戏中遇到问题（请参阅问题 #3 中 FN 所做的说明）。

6. 是否可以拥有多个具有相同所有者但不同头像的`AbilitySystemComponents`（例如，在典当和武器/物品/射弹上将`Owner` 设置为`PlayerState`）？
...
> 
> 不过，棋子和武器上的单独 ASC 本身就有意义。例如，区分描述武器的标签和描述拥有棋子的标签。也许赋予武器的标签也“适用于”所有者而不是其他东西确实有意义（例如，属性和 GE 是独立的，但所有者将像我上面描述的那样聚合拥有的标签）。我确信这会成功。但同一所有者拥有多个 ASC 可能会很冒险。

7. 有没有办法阻止服务器在拥有客户端上覆盖“本地预测能力”的冷却时长？在高延迟下，客户端本地冷却结束但服务器仍在冷却，会导致再次激活时出现额外等待。尤其是冷却极短（例如小于 1 秒）的基础攻击，这个问题更明显。若无法阻止覆盖，Epic 在 Paragon 中是如何减轻高延迟对再次激活能力影响的？
...
> 
> Fortnite 自己记录武器射击频率：它不使用 GE 来计算武器的冷却时间。如果这对您的游戏来说是一个关键问题，我会推荐这样做。

8. Epic 针对 GameplayAbilitySystem 插件的路线图是什么？Epic 计划在 2019 年及以后添加哪些功能？
...
> * 减少 UGameplayAbility 上的“策略”数量。我会删除 ReplicationPolicy 和 InstancingPolicy。在我看来，复制实际上几乎从未被需要，并且会导致混乱。应通过将 FGameplayAbilitySpec 设为可子类化的 UObject 来替换 InstancingPolicy。这应该是具有事件且可蓝图化的“非实例化能力对象”。UGameplayAbility 应该是“每次执行实例化”对象。如果您需要实际实例化，它可以是可选的：相反，“非实例化”能力将通过新的 UGameplayAbilitySpec 对象来实现。
>   *系统应该提供更多的“中层”构造，例如“过滤的GE应用程序容器”（数据驱动哪些GE应用到具有更高级别游戏逻辑的参与者），“重叠体积支持”（基于碰撞原语重叠事件应用“过滤的GE应用程序容器”）等。这些是每个项目最终以自己的方式实现的构建块。让它们正确并非易事，所以我认为我们应该做得更好，提供一些基本的实现。
> * 一般来说，减少项目启动和运行所需的样板文件。可能是一个单独的模块“Ex库”或任何可以提供诸如被动能力或开箱即用的基本命中扫描武器之类的东西。该模块是可选的，但可以让您快速启动并运行。
...
<a id="resources-daveratti-community2"></a>

#### 11.1.2 社区问题 2

社区成员 [iniside](https://github.com/iniside) 与 Dave Ratti 的问答：

1. 是否计划支持“解耦固定 Tick”？例如让游戏线程固定在 30/60fps，而渲染线程自由运行；另外现在已有固定步长的异步物理 Tick，未来系统其余部分会如何配合？

> 目前没有计划把渲染帧率和游戏线程 Tick 频率解耦。考虑到系统复杂度以及对旧版本引擎兼容性的要求，这条路基本上已经不太现实。
> 
> 我们当前的方向是引入异步“物理线程”，它以固定 Tick 率独立运行。需要固定步长的逻辑可以放在这条线程上，而游戏线程和渲染线程继续按现有模式工作。
> 
> 另外补充：Network Prediction 支持 Independent Ticking（独立 Tick）和 Fixed Ticking（固定 Tick）两种模式。长期看，Independent Ticking 会继续保持现在的模式：在游戏线程上以可变帧率运行，并采用经典的“客户端预测自己拥有的 Pawn/Actor”模型。Fixed Ticking 则会更多依托异步物理，用于预测非本地拥有对象（如物理物体、其他客户端的 Pawn/载具等）。

2. Network Prediction 与 Ability System 的整合有没有明确计划？比如基于固定帧进行能力激活与任务执行，而不是依赖 Prediction Key？

> 有计划。方向是重写/移除 Ability System 现有的 Prediction Key 机制，改用 Network Prediction 的结构。`NetworkPredictionExtras` 里的 `MockAbility` 示例展示了思路，不过它们比 GAS 最终需要的形态更“硬编码”。
> 
> 核心思路是：移除 ASC RPC 中显式的“客户端 -> 服务器 Prediction Key 交换”。未来不再强调 prediction window 或 scoped prediction key，而是以 Network Prediction 的帧为锚点。关键是客户端与服务器要对“事件发生时刻”达成一致，比如：
> 
> * 能力何时激活/结束/取消
> * GameplayEffect 何时应用/移除
> * 属性在某一帧的具体数值
> 
> 这些理论上可以在 Ability System 层统一处理。但要让 `UGameplayAbility` 里的用户自定义逻辑完全支持回滚，还需要更多工作。未来可能会有“完全可回滚”的 `UGameplayAbility` 子类，或只开放一组可回滚友好的 AbilityTask。动画事件与根运动也会受影响。
> 
> 现阶段最重要的是先把基础打牢，再去改 GAS：移动和物理必须先足够可靠。

3. Network Prediction 会推进到主分支吗？

> 我们正在推进。系统层面的开发仍在 `NetworkPrediction`（例如 `NetworkPhysics.h`）内进行，底层异步物理能力（如 `RewindData.h`）也在持续完善。我们同时还要满足 Fortnite 的内部需求，因此节奏会受实际项目影响。当前仍在持续修复问题和做性能优化。
> 
> 额外说明：系统早期我们更关注“前端表达方式”（如何声明状态与模拟），后来随着异步物理落地，重心转为“先让真实案例跑起来”，因此暂时牺牲了部分早期抽象。后续会再回头统一前端体验，建立在验证过的底层能力之上。

4. 之前主分支里有过一个用于发送 Gameplay 消息的插件（类似事件总线），后来删掉了。还有计划恢复吗？模块化游戏场景下它会很有价值。

> 你说的应该是 GameplayMessages 插件。它未来可能会回归，但 API 还没最终定型，原作者当时也未计划公开。它对模块化游戏设计确实很有帮助，不过这块不是我直接负责的范围，所以我没有更多时间表信息。

5. 我最近在尝试异步固定物理：需要较多样板代码（尤其插值），并且常常要让整套引擎 Tick 一起“固定化”才好用。未来会不会更容易落地？会扩展到更多框架层，而不是每个项目都自己手写？

> 现在的异步 CharacterMovementComponent 更像早期原型/实验，还不能代表 CMC 的最终形态；目前也没有完整网络支持，因此不建议过早押注。
> 
> 目标方向是：开启异步物理后，引擎可以保持可变 Tick，而物理与“核心游戏模拟”按固定步长运行（例如角色移动、车辆等）。
> 
> 当前可用的一组关键 cvar：
> `p.DefaultAsyncDt=0.03333`
> `p.RewindCaptureNumFrames=64`
> 
> Chaos 已提供物理状态插值（例如将结果推回 `UPrimitiveComponent` 供游戏代码读取），可通过 `p.AsyncInterpolationMultiplier` 观察效果。
> 
> 对于非物理状态的插值，目前仍需项目自行处理。我们知道这会带来大量样板代码。
> 
> 未来希望同时提供两类能力：
> 
> * 面向高级程序员的低层接口（追求极致性能与安全）
> * 更贴近 Unreal 常规工作流的高层接口（例如反射系统与通用蓝图支持），降低手写 `NetSerialize / ShouldReconcile / Interpolate` 的成本
> 
> 现阶段优先级仍是先把关键场景做稳定、做高性能，之后再把“易用性与低样板”补齐。
...
<a id="changelog-5.3"></a>

### 5.3

* 崩溃修复：修复了无缝旅行后尝试应用游戏提示时的崩溃问题。
* 崩溃修复：修复了使用实时编码时由 GlobalAbilityTaskCount 引起的崩溃。
* 崩溃修复：修复了 UAbilityTask::OnDestroy，使其在针对 UAbilityTask_StartAbilityState 等情况递归调用时不会崩溃。
* 错误修复：现在可以安全地在子类中调用`Super::ActivateAbility`。以前，它会调用`CommitAbility`。
* 错误修复：添加了对正确复制不同类型的 FGameplayEffectContext 的支持。
* 错误修复：FGameplayEffectContextHandle 现在将在检索“Actors”之前检查数据是否有效。
* 错误修复：保留游戏能力系统目标数据位置信息的旋转。
* 错误修复：游戏能力系统现在仅在找到有效 PC 时才停止搜索 PC。
* 错误修复：使用现有的 GameplayCueParameters（如果存在）而不是 RemoveGameplayCue_Internal 中的默认参数对象。
* 错误修复：GameplayAbilityWorldReticle 现在面向源 Actor，而不是 TargetingActor。
* 错误修复：如果使用 GiveAbilityAndActivateOnce 传入并且能力列表被锁定，则缓存触发事件数据。
* 错误修复：添加了对 FInheritedGameplayTags 的支持，以立即更新其 CombinedTags，而不是等到保存。
* 错误修复：将 ShouldAbilityRespondToEvent 从仅限客户端的代码路径移至服务器和客户端。
* 错误修复：修复了 FAttributeSetInitterDiscreteLevels 由于曲线简化而无法在烘焙版本中工作的问题。
* 错误修复：在 GameplayAbility 中设置 CurrentEventData。
* 错误修复：确保在可能执行回调之前正确设置 MinimalReplicationTags。
* 错误修复：修复了 ShouldAbilityRespondToEvent 未在实例化 GameplayAbility 上调用的问题。
* 错误修复：禁用 gc.PendingKill 时，在子 Actor 上执行的 Gameplay Cue Notify Actor 不再泄漏内存。
* 错误修复：修复了 GameplayCueManager 中的一个问题，其中 GameplayCueNotify_Actors 可能由于哈希冲突而“丢失”。
* 错误修复：即使我们在 Actor 上没有游戏标签，WaitGameplayTagQuery 现在也会尊重其查询。
* 错误修复：PostAttributeChange 和 AttributeValueChangeDelegates 现在将具有正确的 OldValue。
* 错误修复：修复了 FGameplayTagQuery 如果该结构是由本机代码创建的，则不会显示正确的查询描述的问题。
* 错误修复：确保在使用能力系统时调用 UAbilitySystemGlobals::InitGlobalData。以前如果用户不调用它，游戏能力系统就无法正常运行。
* 错误修复：修复了从 UGameplayAbility::EndAbility 链接/取消链接动画层时的问题。
* 错误修复：更新了能力系统组件功能，以在使用前检查规格的能力指针。
* 新增内容：向 FGameplayTagRequirements 添加了 GameplayTagQuery 字段，以便可以指定更复杂的要求。
* 新增内容：引入了 FGameplayEffectQuery::SourceAggregateTagQuery 来增强 SourceTagQuery。
* 新增内容：扩展了从控制台命令执行和取消游戏能力和游戏效果的功能。
* 新增内容：添加了对游戏能力蓝图执行“审核”的功能，该蓝图将显示有关如何开发和打算使用它们的信息。
* 更改：现在在主实例上调用 OnAvatarSet，而不是在按 Actor 游戏能力实例化的 CDO 上调用。
* 更改：允许在同一个游戏能力图中同时激活能力和从事件激活能力。
* 更改：AnimTask_PlayMontageAndWait 现在有一个切换功能，允许在 BlendOut 事件后完成和中断。
* 更改：ModMagnitudeCalc 包装函数已声明为 const。
* 更改：FGameplayTagQuery::Matches 现在对于空查询返回 false。
* 更改：更新了 FGameplayAttribute::PostSerialize 以将包含的属性标记为可搜索名称。
* 更改：将 `GetAbilitySystemComponent` 更新为默认参数 `Self`。
* 更改：在 AbilityTask_WaitTargetData 中将函数标记为虚拟函数。
* 更改：删除了未使用的函数 FGameplayAbilityTargetData::AddTargetDataToGameplayCueParameters。
* 更改：删除了残留的 GameplayAbility::SetMovementSyncPoint。
* 更改：从游戏任务和能力系统组件中删除了未使用的复制标志。
* 更改：将部分 GameplayEffect 功能迁移到可选组件中；如有需要，现有内容会在 `PostCDOCompiled` 期间自动更新为组件用法。

https://docs.unrealengine.com/5.3/en-US/unreal-engine-5.3-release-notes/

<a id="changelog-5.2"></a>
...
* 错误修复：修复了链式调用 `WaitGameplayEvent` 时的内存破坏问题。
* 错误修复：当多次执行同一节点时，在蓝图中调用 `AbilitySystemComponent::GetOwnedGameplayTags()` 函数不再保留先前调用的返回值。
...
* 新功能：[游戏瞄准系统](https://docs.unrealengine.com/en-US/gameplay-targeting-system-in-unreal-engine/)是一种创建数据驱动的目标请求的方法。
...
* 新增内容：游戏能力系统属性现在尊重核心重定向。这意味着您现在可以在代码中重命名属性集及其属性，并通过向 DefaultEngine.ini 添加重定向条目，将它们正确加载到使用旧名称保存的资产中。
* 更改：允许从代码更改游戏效果修改器的评估通道。
* 更改：从游戏能力插件中删除了之前未使用的变量`FGameplayModifierInfo::Magnitude`。
* 更改：删除了能力系统组件和智能对象实例标签之间的同步逻辑。

https://docs.unrealengine.com/5.2/en-US/unreal-engine-5.2-release-notes/

<a id="changelog-5.1"></a>
...
* 新增功能：修改了控制台变量中的 AbilityTask 调试，以在非发布版本中默认启用调试记录和打印（能够根据需要打开/关闭热修复功能）。
* 新增内容：您现在可以将 AbilitySystem.AbilityTask.Debug.RecordingEnabled 设置为 0 来禁用，设置为 1 来在非发布版本中启用，设置为 2 来启用所有构建（包括发布）。
* 新增内容：您可以使用AbilitySystem.AbilityTask.Debug.AbilityTaskDebugPrintTopNResults 仅打印日志中的前N 个结果（以避免日志垃圾邮件）。
* 新增功能：STAT_AbilityTaskDebugRecording 可用于测试这些默认调试更改对性能的影响。
* 新增内容：添加了一个调试命令来过滤 GameplayCue 事件。
* 新增内容：向 Gameplay Ability System 添加了新的调试命令：`AbilitySystem.DebugAbilityTags`、`AbilitySystem.DebugBlockedTags` 和 `AbilitySystem.DebugAttribute`。
...
* 更改：`GameplayTasksComponent` 和 `AbilitySystemComponent` 现在支持注册子对象 API。
* 更改：添加了更好的日志记录以指示游戏功能无法激活的原因。
* 更改：删除了 AbilitySystem.Debug.NextTarget 和 PrevTarget 命令，以支持全局 HUD NextDebugTarget 和 PrevDebugTarget 命令。
...
* 错误修复：修复了 GameplayEffect 混合复制模式中的一个边缘情况，其中未明确拥有网络连接但使用来自 `GetNetConnection` 的连接的 Actor 将不会收到混合复制更新。
* 错误修复：修复了 GameplayAbility 的类方法 `EndAbility` 中发生的无限递归，该方法是通过从 `K2_OnEndAbility` 再次调用 `EndAbility` 来调用的。
...
* 新增内容：本机游戏标签。引入了新的`FNativeGameplayTag`，使得在加载和卸载模块时正确注册和取消注册的一次性本机标签成为可能。
* 新增内容：更新了 `GiveAbilityAndActivateOnce` 以传入 FGameplayEventData 参数。
...
* 新增内容：添加了更多支持，以使用特定于项目的过滤器在编辑器中过滤 GameplayTags。`OnFilterGameplayTag` 提供引用属性和标签源，因此您可以根据请求标签的资产来过滤标签。
* 新增内容：添加了在初始化后调用 GameplayEffectSpec 的类方法 `SetContext` 时保留原始捕获的 SourceTags 的选项。
* 新增内容：改进了用于从特定插件注册 GameplayTags 的 UI。新的标签 UI 现在允许您为新添加的 GameplayTag 源选择磁盘上的插件位置。
